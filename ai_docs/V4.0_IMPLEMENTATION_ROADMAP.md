...
**期間**: Late Nov 2025 - Q1 2026 (4-5 weeks)
**Status**: ✅ **Foundation Complete** (#382), 🔄 **Remote MCP In Progress** (#378)
...
3. 🔜 **Memory Consolidation** - Export/Import, lifecycle management
...
**2. 🔄 Remote MCP Server** (Issue #378 - **IN PROGRESS**):
- Approach: **Option 2 (Hybrid)** - Local MCP Connector + Remote REST API
- Local connector: stdio ↔ HTTP bridge
- API authentication: API Key (leverages user_id from #382)
...
**🔄 In Progress**:
- Issue #378: Remote MCP Server (詳細は Issue #378 を参照)

**Week 1-2**: Remote MCP Server
- MCP Connector implementation
- API Key authentication (using user_id from #382)
- Tool access control
