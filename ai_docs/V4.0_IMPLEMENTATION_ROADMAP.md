...
## ✅ Phase C+: Consolidation & Performance (v4.1.0-v4.1.1) 🆕

**期間**: Nov 2025 - Jan 2026
**Status**: ✅ **COMPLETE**
**Releases**: v4.1.0 (Nov 5), v4.1.1 (Jan 6, 2026)

### v4.1.0: Code Quality & CLI Improvements
**Issues**: #549, #550, #539, #545, #546
**完了日**: 2025-11-05

**実装内容**:
- Shared CLI utilities (`cli/utils/`: progress, rich_helpers, time_formatters)
- API connectivity testing consolidation (`utils/api_check.py`)
- Unified error handling (`utils/errors.py`)
- CLI command deprecation準備

**成果**: +500 lines, コード重複削減, 保守性向上

### v4.1.1: Performance & Context Awareness
**Issues**: #548, #555, #411, #400
**完了日**: 2025-11-06

**主要機能**:

1. **CLI Performance (#548)**: 83% faster startup (13.9s → 2.3s)
   - Lightweight config pattern
   - Fixed `model_post_init()` auto-detection bug
   - Performance regression tests

2. **CLI Reorganization (#555)**: Better UX
   - `kagura config setup` - Interactive wizard
   - `kagura mcp telemetry` - Command grouping
   - Breaking: Removed `telemetry` and `config show`

3. **Enhanced Memory Stats (#411)**: Usage insights
   - Unused memory tracking (30/90 days)
   - Storage calculation (MB)
   - Automatic access tracking

4. **MCP Middleware (#400)**: Context awareness
   - Auto-logging of tool calls
   - `memory_get_tool_history` tool
   - Fire-and-forget implementation
   - Recursion prevention

**Branch Workflow**: Release branch pattern adopted

**成果**: +1,500 lines, +23 tests, 83% performance gain

---

## 🌐 Phase D-F: Self-hosted API + Multimodal MVP + Consumer App

**期間**: Q2 2026 (3 months)
**Goal**: Multimodal features、Advanced connectors
