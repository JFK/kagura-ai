...
### 完了済み (v3.0.8 - 2025-10-23)
- ✅ Streaming support
- ✅ User config (`kagura init`)
- ✅ Personal tools (4個)
- ✅ Context compression
- ✅ MCP integration
- ✅ Memory RAG auto-detect (#354, #356)
- ✅ MCP tool descriptions強化 (#355, #357, #358, #359)
- ✅ Cross-thread memory sharing guidance
- ✅ memory_list debugging tool (#358)
...
### Integration Testでstr/LLMResponse両方を扱う

v3.0.2以降、`@agent`の返り値は型アノテーションによって異なります。Integration testではこれを考慮：

```python
@pytest.mark.integration
@pytest.mark.asyncio
async def test_my_agent():
    """Test agent that may return str or LLMResponse"""
    result = await my_agent("test input")

    # Handle both return types
    if isinstance(result, str):
        # Agent has -> str annotation (returns plain string)
        assert len(result) > 0
...
**理由**:
- `-> str`アノテーション → `LLMResponse.content`を直接返す
- アノテーション無し → `LLMResponse`オブジェクトを返す
- Testは両方に対応することで柔軟性を確保

### CI環境でのpyright設定

pyrightがCI環境で遅延する場合、設定を最適化：

```json
// pyrightconfig.json
{
  "include": ["src/kagura"],  // プロジェクト全体をカバー
...
**ポイント**:
- `include`でプロジェクト全体を指定（特定ディレクトリではなく）
- CI実行コマンドは`pyright`（引数なし、configに従う）
- `pyright src/kagura/`のように明示的パスを避ける
- 最新版pyrightを使用（パフォーマンス改善）

**CI timeout対策**:
```yaml
# .github/workflows/test.yml
- name: Run type checking
  run: uv run pyright  # No args, follows config
  timeout-minutes: 3   # Local: ~5s, CI: with margin
```
