...
**⛔️ 重要**: mainブランチへの直接コミットは**絶対に禁止**です。

**必須ルール**:
- ✅ すべての変更はPR（Pull Request）経由でマージ
- ✅ 変更前に必ずIssue作成 → ブランチ作成 → Draft PR
- ✅ CI/CDチェック（テスト、型チェック、リント）がすべて通過
- ❌ `git push origin main` は禁止
- ❌ `git commit --amend` on main は禁止

**例外ケース**（極めて限定的）:
- 🚨 Production緊急修正（Hotfix）のみ
  - この場合も、修正後に即座にPRを作成し、レビューを受けること
  - 修正内容をIssueに記録すること

**理由**:
- コードレビュープロセスの必須化
- CI/CDによる品質保証
- トレーサビリティの確保（Issue-PR-Commit連携）
- ロールバックの容易性
